...
const AdicionarProdutoModal = ({ isOpen, produto, modo = "entrada", onConfirm, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      setQuantidade("");
      setValorUnitario(
        isSaida ? String(Number(valorUnitarioSaida).toFixed(2)) : ""
      );
    }
  ...
  const valorUnit = isSaida
    ? Number(valorUnitarioSaida)
          ...
          <div className="p-5 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                value={
                  isSaida
                    ? Number(valorUnitarioSaida).toFixed(2)
                    : valorUnitario
                }
                onChange={(e) => !isSaida && setValorUnitario(e.target.value)}
                className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring focus:ring-blue-200 ${
                  isSaida ? "bg-gray-100 cursor-not-allowed" : ""
                }`}
                placeholder={
                  isSaida
              ...
              {isSaida && (
                <p className="text-xs text-gray-500 mt-1">
                  O valor unitário é definido automaticamente pelo custo
                  médio do produto.
                </p>
              )}
            </div>
